...
import sys
sys.path.append("../")
from utils_freq.freq_fourier_loss import  fft_L1_loss_color,decide_circle, fft_L1_loss_mask, calc_fft
from utils_freq.freq_pixel_loss import find_fake_freq, get_gaussian_kernel
# from stylegan2 import model

from model2 import Generator, Discriminator, CooccurDiscriminator

# from model import Encoder, Generator, Discriminator, CooccurDiscriminator
# from model2 import Encoder, Generator, Discriminator, CooccurDiscriminator
...
class MyDataset(Dataset):
    ...
    def __init__(self, list_file, transform=None):
        f= open(list_file,"r")
        # print("f = ", f)
        print("data_lines =", f.readlines())
        # f = os.path.join(args.dataset_txt, 'train')
...
def train(
    ...
    mask_h, mask_l = decide_circle(r=args.radius, N=int(args.batch), L=args.size)
    mask_h, mask_l = mask_h.cuda(), mask_l.cuda()

    loader1 = sample_data(loader1)
    ...
    if get_rank() == 0:
        ...
        os.makedirs('h_img_'+str(args.h_img), exist_ok= True)
        os.makedirs('o_img_'+str(args.h_img), exist_ok= True)
    ...
    if args.resume:
      ...
      e_module.load_state_dict(checkpoint['e'])
      g_module.load_state_dict(checkpoint['g'])
      d_module.load_state_dict(checkpoint['d'])
      c_module.load_state_dict(checkpoint['cooccur'])
      e_ema.load_state_dict(checkpoint['e_ema'])
    ...
    for idx in pbar:
        ...
        real_img1 = next(loader1)
        real_img2 = next(loader2)

        real_img1 = real_img1.to(device)
        real_img2 = real_img2.to(device)

        real_img_freq1 = find_fake_freq(real_img1, gauss_kernel)
        real_img_freq2 = find_fake_freq(real_img2, gauss_kernel)
        # pdb.set_trace()
        ...
        fake_img1 = generator(real_img1, real_img2, s=1)
        fake_img2 = generator(real_img1, real_img2, s=0)

        fake_pred = discriminator(torch.cat((fake_img1, fake_img2), 0))

        real_img = torch.cat((real_img1, real_img2), 0)

        real_pred = discriminator(real_img)
        d_loss = d_logistic_loss(real_pred, fake_pred)

        fake_img2_freq = find_fake_freq(fake_img2, gauss_kernel)
        fake_patch = patchify_image(fake_img2_freq[:, 3:6, :, :], args.n_crop)
        real_patch = patchify_image(real_img_freq2[:, 3:6, :, :], args.n_crop)
        ref_patch = patchify_image(real_img_freq2[:, 3:6, :, :], args.ref_crop * args.n_crop)
        ...
        # utils.save_image(fake_img2, '/content/drive/MyDrive/frequency-domain-image-translation/swapping-autoencoder/adv_img', normalize=True, range=(-1, 1))
        # print("fake_size =", fake_img2.size())
        # print("real_size =", real_img1.size())
        ...
        # fake_img_r = fake_img2_freq[:, 3:6, :, :] - fake_img1_freq[:, 3:6, :, :]
        # fake_img_d = fake_img2_freq[:, 3:6, :, :] - fake_img_r


        recon_freq_loss_img2_structure = F.l1_loss(fake_img2_freq[:, 3:6, :, :], real_img_freq1[:, 3:6, :, :])
        # recon_freq_loss_img2_structure = F.l1_loss(fake_img_d, real_img_freq1[:, 3:6, :, :])
        ...
        if get_rank() == 0:
            ...
            '''
            sampleee = torch.cat((real_img2, real_img_freq2[:, :3, :, :]), 0)
            utils.save_image(
                        sampleee,
                        f"h_img_{str(args.h_img)}/{str(i).zfill(6)}.png",
                        nrow=int(sampleee.shape[0] / 4 ),
                        normalize=True,
                        range=(-1, 1),
                    )
            ...
            real_patch = patchify_image(real_img_freq2[:, :3, :, :], args.n_crop)
            ref_patch = patchify_image(real_img_freq2[:, :3, :, :], args.ref_crop * args.n_crop)

            a = calc_fft(fake_img2)
            b = calc_fft(fake_img_r)
            c = calc_fft(fake_img_d)
            d = calc_fft(fake_img1)
            e = c - d

            samplee = torch.cat((fake_patch, real_patch, ref_patch, real_patch), 0) 
            utils.save_image(
                        samplee,
                        f"o_img_{str(args.o_img)}/{str(i).zfill(6)}.png",
                        nrow=int(samplee.shape[0] / 4 ),
                        normalize=True,
                        range=(-1, 1),
                    )                    
            ...
            if i % 100 == 0:
                with torch.no_grad():
                    ...
                    fake_img1 = g_ema(real_img1, real_img2, s=1)
                    fake_img2 = g_ema(real_img1, real_img2, s=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("path1", type=str, nargs="+", default=' ')
    parser.add_argument("path2", type=str, nargs="+", default=' ')
    ...
    parser.add_argument("--h_img", type=str, default="h_img")
    parser.add_argument("--o_img", type=str, default="o_img")
    ...
    # parser.add_argument('--dataset_txt', '-d', type=str, default=None)
    ...
    # args.dataset_txt = '/content/drive/MyDrive/frequency-domain-image-translation/swapping-autoencoder/flickr'
    # print("data =", os.path.isfile(args.dataset_txt))
    #print("path =", args.path1)
    #print("path =", args.path2)
    ...
    if  args.dataset_txt == None:
        ...
        for path1 in args.path1:
            dataset = MultiResolutionDataset(path1, transform, args.size)
        ...
        for path2 in args.path2:
            dataset = MultiResolutionDataset(path2, transform, args.size)
            datasets2.append(dataset)

        loader2 = data.DataLoader(
            data.ConcatDataset(datasets2),
            batch_size=args.batch,
            sampler=data_sampler(dataset, shuffle=True, distributed=args.distributed),
            drop_last=True,
        )
